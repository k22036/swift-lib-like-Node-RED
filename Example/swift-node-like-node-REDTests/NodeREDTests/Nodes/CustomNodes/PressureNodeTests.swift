import Foundation
import Testing

@testable import swift_lib_like_Node_RED

struct PressureNodeTests {
    @Test func parse() async throws {
        let jsonString = """
            [
                {
                    "id": "prs1",
                    "type": "pressure",
                    "z": "test-z",
                    "name": "prs test",
                    "repeat": "1.0",
                    "once": true,
                    "onceDelay": "0.1",
                    "x": 10,
                    "y": 20,
                    "wires": [["node2"]]
                }
            ]
            """
        guard let data = jsonString.data(using: .utf8) else {
            fatalError("Failed to convert JSON string to Data")
        }
        let nodes = try JSONDecoder().decode([PressureNode].self, from: data)
        guard let node = nodes.first else {
            fatalError("No PressureNode found in JSON data")
        }
        #expect(node.id == "prs1")
        #expect(node.type == "pressure")
        #expect(node.z == "test-z")
        #expect(node.name == "prs test")
        #expect(node.repeat == 1.0)
        #expect(node.once == true)
        #expect(node.onceDelay == 0.1)
        #expect(node.wires.first?.first == "node2")
    }

    @Test func simulatePressure() async throws {
        let jsonString = """
            [
                {
                    "id": "prs2",
                    "type": "pressure",
                    "z": "test-z",
                    "name": "",
                    "repeat": "",
                    "once": false,
                    "onceDelay": 0,
                    "x": 0,
                    "y": 0,
                    "wires": [["test-node"]]
                }
            ]
            """
        guard let data = jsonString.data(using: .utf8) else {
            fatalError("Failed to convert JSON string to Data")
        }
        let node = try JSONDecoder().decode([PressureNode].self, from: data).first!
        let testNode = try TestNode(id: "test-node")
        let flow = try await Flow(flowJson: "[]")
        await flow.addNode(node)
        await flow.addNode(testNode)

        await node.initialize(flow: flow)
        await node.simulatePressure(1005.5)
        try await Task.sleep(nanoseconds: UInt64(0.1 * 1_000_000_000))
        await node.terminate()

        #expect(await testNode.buffer.count == 1)
        if let msg = await testNode.buffer.first {
            guard let payload = msg.payload as? [String: Double] else {
                fatalError("Payload is not a dictionary")
            }
            #expect(payload["pressure"] == 1005.5)
        }
    }
}
